...
const InstructorsDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user && user.email) {
      ...
      const url = `http://localhost:4000/instructor/get_instructor_profile?email=${encodeURIComponent(user.email)}`;
  ...
  const handleUpdate = () => {
    ...
    fetch('http://localhost:4000/instructor/update_instructor_profile', {
      method: 'PUT',
